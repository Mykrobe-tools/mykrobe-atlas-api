...
const fixturePublicElasticsearchResponse = {
  _shards: {
    failed: 0,
    skipped: 0,
    successful: 1,
...
const fixturePrivateElasticsearchResponse = {
  _shards: {
    failed: 0,
    skipped: 0,
    successful: 1,
    ...
    hits: [
      {
        ...
        _source: {
          ...
          sampleId: "829c1263-d492-4143-b248-ed668394a7b9",
          owner: {
            id: "ee4a9c35-630b-4e17-9601-d5fbf1ed8b1d"
          }
        },
        _type: "_doc"
      }
...
describe("ExperimentsResultJSONTransformer", () => {
  describe("#transform", () => {
    describe("when valid", () => {
      describe("when transforming a public experiment", () => {
        let first = null;
        beforeEach(done => {
          const transformer = new ExperimentsResultJSONTransformer();
          const json = transformer.transform(fixturePublicElasticsearchResponse);
          first = json[0];
          done();
        });
        ...
        it("should set owner to false", done => {
          expect(first).toHaveProperty("owner", false);
          done();
        });
        it("should set public to true", done => {
          expect(first).toHaveProperty("public", true);
          done();
        });
      });
      describe("when transforming a private experiment", () => {
        describe("when not passed a currentUser", () => {
          let first = null;
          beforeEach(done => {
            const transformer = new ExperimentsResultJSONTransformer();
            const json = transformer.transform(fixturePrivateElasticsearchResponse);
            first = json[0];
            done();
          });
          it("should set owner to false", done => {
          ...
          it("should set public to true", done => {
            expect(first).toHaveProperty("public", false);
            done();
          });
        });
        describe("when passed a currentUser", () => {
          describe("when the user id matches", () => {
            let first = null;
            beforeEach(done => {
              const transformer = new ExperimentsResultJSONTransformer();
              const json = transformer.transform(fixturePrivateElasticsearchResponse, {
                currentUser: { id: "ee4a9c35-630b-4e17-9601-d5fbf1ed8b1d" }
              });
              first = json[0];
              done();
            });
            it("should set owner to true", done => {
              expect(first).toHaveProperty("owner", true);
              done();
            });
            it("should set public to false", done => {
              expect(first).toHaveProperty("public", false);
              done();
            });
          });
          describe("when the user id does not match", () => {
            let first = null;
            beforeEach(done => {
              const transformer = new ExperimentsResultJSONTransformer();
              const json = transformer.transform(fixturePrivateElasticsearchResponse, {
                currentUser: { id: "fa567c35-630b-4e17-9601-d5fbf1ed81ae" }
              });
              first = json[0];
              done();
            });
