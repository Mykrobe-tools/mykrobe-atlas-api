...
describe('## User Functions', () => {
  ...
  it('should return an error if not found', (done) => {
    User.get('58d3f3795d34d121805fdc61')
      .catch((e) => {
        expect(e.name).to.equal('ObjectNotFound');
        expect(e.message).to.equal('User not found with id 58d3f3795d34d121805fdc61');
        done();
      });
  });
  ...
  it('should fetch the user by phone', (done) => {
    User.getByEmail('thomas@nhs.co.uk')
      .then((user) => {
        expect(user.firstname).to.equal('Thomas');
        expect(user.lastname).to.equal('Carlos');
        done();
  ...
  it('should return an error if email not found', (done) => {
    User.getByEmail('amir@nhs.co.uk')
      .catch((e) => {
        expect(e.name).to.equal('ObjectNotFound');
  ...
  it('should find and update the user', (done) => {
    User.findUserAndUpdate({ firstname: 'Thomas' }, { lastname: 'Vardy' })
      .then((user) => {
        expect(user.firstname).to.equal('Thomas');
  ...
  it('should return an error if the user was not found', (done) => {
    User.findUserAndUpdate({ firstname: 'James' }, { lastname: 'Vardy' })
      .catch((e) => {
        expect(e.name).to.equal('ObjectNotFound');
        expect(e.message).to.equal('No registered user with the given criteria');
        done();
      });
  });
  it('should find the user by resetPasswordToken', (done) => {
    User.getByResetPasswordToken('54VwcGr65AKaules/blueln7w1o')
      .then((user) => {
        expect(user.firstname).to.equal('Thomas');
        expect(user.lastname).to.equal('Carlos');
        done();
  ...
  it('should return an error if resetPasswordToken doesnt exist', (done) => {
    User.getByResetPasswordToken('000000')
      .catch((e) => {
        expect(e.name).to.equal('ObjectNotFound');
  ...
  it('should find the user by verificationToken', (done) => {
    User.getByVerificationToken(107166)
      .then((user) => {
        expect(user.firstname).to.equal('Thomas');
        expect(user.lastname).to.equal('Carlos');
        done();
  ...
  it('should return an error if combination doesnt match', (done) => {
    User.getByVerificationToken(107100)
      .catch((e) => {
        expect(e.name).to.equal('ObjectNotFound');
