...
function login(req, res) {
  ...
  User.findOne({
    ...
    if (user && passwordHash.verify(req.body.password, user.password)) {
      ...
      return res.jerror(new APIError('You must validate your account first', httpStatus.UNAUTHORIZED));
    }
    return res.jerror(new APIError('Invalid credentials supplied', httpStatus.UNAUTHORIZED));
  });
}
...
function forgot(req, res) {
  ...
  User.findUserAndUpdate({ email: req.body.email }, { resetPasswordToken: random })
    .then((user) => {
      const queue = config.monqClient.queue(config.notification);
      queue.enqueue('forgot', { to: user.email, token: random }, () => res.jsend(`Email sent successfully to ${user.email}`));
    })
...
function reset(req, res) {
  User.getByResetPasswordToken(req.body.resetPasswordToken)
    .then((user) => {
      ...
      user.save()
        .then(savedUser => res.jsend(`Password was reset successfully for ${savedUser.email}`))
...
function resend(req, res) {
  ...
  User.getByEmail(email)
    ...
    .then((userWithToken) => {
      const queue = config.monqClient.queue(config.notification);
      queue.enqueue('welcome', { token: userWithToken.verificationToken, to: userWithToken.email }, () => res.jsend(`Notification was resent by ${config.notification}`));
    })
    .catch(e => res.jerror(e));
}
